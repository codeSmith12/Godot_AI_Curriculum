...
func _physics_process(delta: float) -> void:
	...
	if world.demo_ai and not world.external_training:
		world.last_probabilities = world.policy.probabilities(ai_controller.get_obs().obs)
		should_flap = world.last_probabilities[1] > world.last_probabilities[0]
	elif ai_controller.heuristic == "human":
